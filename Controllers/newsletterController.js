    ...
    .map(async () => {
      while (idx < items.length) {
        ...
        try {
          ...
          results[current] = { success: false, error: err?.message || 'Unknown error' };
        }
      }
    });
...
exports.createNewsletter = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Server error creating newsletter.' });
  }
};
...
exports.sendNewsletter = async (req, res) => {
  ...
  try {
    ...
    if (newsletter.status === 'sent') {
      ...
      return res.status(409).json({ success: false, message: 'Newsletter already marked as sent.' });
    }
    ...
    console.info(`[NEWSLETTER][SEND][START] id=${newsletter._id}, subject="${newsletter.subject}", recipients=${normalized.length}, dryRun=${dryRun}`);
    ...
    newsletter.status = failed === 0 && !dryRun ? 'sent' : dryRun ? 'dry-sent' : 'partial';
    ...
    console.info(`[NEWSLETTER][SEND][END] id=${newsletter._id}, total=${normalized.length}, sent=${sent}, failed=${failed}, dryRun=${dryRun}`);
    ...
    return res.status(500).json({ success: false, message: 'Server error while sending newsletter.' });
  }
}
