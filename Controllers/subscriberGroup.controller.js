...
const SubscriberGroup = require("../Models/SubscriberGroupSchema");
 ...
 */
exports.createSubscriberGroup = async (req, res) => {
  try {
    ...
    const existing = await SubscriberGroup.findOne({
      groupId: id.trim(),
    }).lean();
    if (existing) {
      return res
        .status(409)
        .json({ error: "Subscriber group with this id already exists." });
    }
    ...
    if (err?.code === 11000) {
      return res
        .status(409)
        .json({ error: "Duplicate key: 'id' must be unique." });
    }
 ...
 */
exports.updateSubscriberGroup = async (req, res) => {
  try {
    const currentExternalId = req.params?.id?.trim();
    if (!currentExternalId) {
      return res
        .status(400)
        .json({ error: "Path parameter ':id' is required." });
    }
    ...
    if (typeof name !== "undefined") {
      if (typeof name !== "string" || !name.trim()) {
        return res
          .status(400)
          .json({ error: "If provided, 'name' must be a non-empty string." });
      }
    ...
    if (typeof newExternalId !== "undefined") {
      if (typeof newExternalId !== "string" || !newExternalId.trim()) {
        return res
          .status(400)
          .json({ error: "If provided, 'id' must be a non-empty string." });
      }
      update.groupId = newExternalId.trim();

      // Prevent conflict with another record's groupId
      const currentDoc = await SubscriberGroup.findOne({
      ...
      if (conflicting) {
        return res
          .status(409)
          .json({ error: "Another group already uses this 'id'." });
      }
    }

    if (Object.keys(update).length === 0) {
      return res.status(400).json({
        error: "No valid fields to update. Provide 'name' and/or 'id'.",
      });
    }
