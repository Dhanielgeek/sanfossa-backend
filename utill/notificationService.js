...
exports.notifySubscribers = async (type, item) => {
    try {
        ...
        let subject = '';
        let htmlBody = '';
        const itemUrl = `${process.env.CLIENT_URL}/${type.toLowerCase()}/${item._id}`;
        const itemImage = item.image || item.imageUrl || 'https://default-image-url.com/placeholder.jpg';
        ...
        if (type === 'BLOG') {
            ...
            htmlBody = `
                ...
                <p>We've added a new book to our inventory: <strong>${item.title}</strong> by ${item.author}.</p>
        ...
        console.log(`Successfully notified ${subscriberEmails.length} subscribers.`);
